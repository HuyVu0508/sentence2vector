...
def load_rt(loc='./data/'):
    ...
    """
    pos, neg = [], []
    with open(os.path.join(loc, 'rt-polarity.pos'), 'r') as f:
        for line in f:
            pos.append(line.decode('latin-1').strip())
            # pos.append(line.strip())
    with open(os.path.join(loc, 'rt-polarity.neg'), 'r') as f:
        for line in f:
...
def load_cr(loc='./data/'):
    ...
    """
    pos, neg = [], []
    with open(os.path.join(loc, 'custrev.pos'), 'r') as f:
        for line in f:
            text = line.strip()
            if len(text) > 0:
                pos.append(text)
    with open(os.path.join(loc, 'custrev.neg'), 'r') as f:
        for line in f:
            text = line.strip()
            if len(text) > 0:
                neg.append(text)

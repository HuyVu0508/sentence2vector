...
tf.flags.DEFINE_string("model_version", None, "Determining running model: 0 - original model, 1 - improved model")
tf.flags.DEFINE_string("input_file_pattern_word", None, "File input_file_pattern_POS of sharded TFRecord files containing")
tf.flags.DEFINE_string("input_file_pattern_POS", None, "File pattern of sharded TFRecord files containing")
...
####
FLAGS.model_version = 1 # Determining running model: 0 - original model, 1 - improved model
####
FLAGS.input_file_pattern_word = "../UMBC corpus TFRecords/TFRecords_words/train-?????-of-00100"
FLAGS.input_file_pattern_POS = "../UMBC corpus TFRecords/TFRecords_POSs/train-?????-of-00100"
FLAGS.batch_size = 200
FLAGS.uniform_init_scale = 0.1
FLAGS.shuffle_input_data = False
FLAGS.input_queue_capacity = 640000
FLAGS.num_input_reader_threads = 1
FLAGS.learning_rate = 0.001  #0.0005
FLAGS.learning_rate_decay_steps = 400000
FLAGS.clip_gradient_norm = 5.0
FLAGS.save_model_secs = 60*15 #1800
FLAGS.save_summaries_secs = 300 # 600
FLAGS.nepochs = 1
FLAGS.num_train_inst = 45786400
FLAGS.sequence_length = 30
FLAGS.context_size = 1
FLAGS.dropout = False
FLAGS.dropout_rate = 0.3
FLAGS.model_config = "../model_configs/BS400-W300-S1000-Glove-UMBC-bidir/train.json"
FLAGS.max_ckpts = 5
FLAGS.Glove_path = "dictionaries/Glove"
if FLAGS.model_version==0:   # Running original model
    FLAGS.train_dir = "results/BS400-W300-S1000-Glove-UMBC-bidir/train/original model"   
elif FLAGS.model_version==1: # Running improved model
    FLAGS.train_dir = "results/BS400-W300-S1000-Glove-UMBC-bidir/train/improved model"
...
def main(unused_argv):
  if ((not FLAGS.input_file_pattern_word) | (not FLAGS.input_file_pattern_POS)):
    print("Hello")  
  ...
  g = tf.Graph()
  with g.as_default():      
    model = s2v_model_200.s2v(model_config, mode="train")
    model.build()
